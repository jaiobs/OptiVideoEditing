package com.litpic.litpicsdkdroid.camerapreview.cameraview.frame;
...
public abstract class FrameManager<T> {
    ...
    public void setUp(int format, @NonNull Size size, @NonNull Angles angles) {
        ...
        mFrameSize = size;
        mFrameFormat = format;
        long bitsPerPixel = ImageFormat.getBitsPerPixel(format);
        long sizeInBits = size.getHeight() * size.getWidth() * bitsPerPixel;
        mFrameBytes = (int) Math.ceil(sizeInBits / 8.0d);
        for (int i = 0; i < getPoolSize(); i++) {
            boolean flag = mFrameQueue.offer(new Frame(this));
            Log.d("Litpic-CameraSdk","flag -> "+flag);
        }
        mAngles = angles;
    }
