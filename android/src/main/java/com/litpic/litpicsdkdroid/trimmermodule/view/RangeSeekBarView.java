...
public class RangeSeekBarView extends View {
    ...
    private void init() {
        ...
        mScaleRangeMax = 100;
        mHeightTimeLine = getContext().getResources().getDimensionPixelOffset(R.dimen.frames_video_height_medium);
    ...
    @Override
    public boolean onTouchEvent(@NonNull MotionEvent ev) {              // NOSONAR
        final Thumb mThumb;
        final Thumb mThumb2;
    ...
    public float scaleToPixel(float scaleValue){
        return (scaleValue * (mPixelRangeMax - (2 * mThumbWidth))) / 100;
    }

    public float getScaleMaxRangeValue(int scaleValue){
        return (scaleValue * mPixelRangeMax) / 100;
    }
    ...
    public void setThumbPosition(int index, float pos) {
        mThumbs.get(index).setPos(scaleToPixel(index, pos));
    ...
    private void drawThumbs(@NonNull Canvas canvas) {
        ...
        if (!mThumbs.isEmpty()) {
            for (Thumb th : mThumbs) {
                if (th.getIndex() == 0) {
                    canvas.drawBitmap(Bitmap.createScaledBitmap(th.getBitmap(), th.getBitmap().getWidth(), mHeightTimeLine, false), th.getPos() + getPaddingLeft(), getPaddingTop(), null);
                } else {
                    canvas.drawBitmap(Bitmap.createScaledBitmap(th.getBitmap(), th.getBitmap().getWidth(), mHeightTimeLine, false), th.getPos() - getPaddingRight(), getPaddingTop(), null);
                }
            }
        }
    }
