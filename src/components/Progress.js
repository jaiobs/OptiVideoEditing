...
class ProgressBar extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if(this.props.stopAnimation === true){
      this.stopAllAnimations()
    }
  }

  animateWidth() {
    let toValue = ((this.props.width * this.state.progress) / 100) - this.props.borderWidth * 2;
    toValue = this.state.progress+this.props.width/30
  ...
  render() {
    ...
    return (
      ...
      >
        <Animated.View 
              onLayout={(event) => {this.props.onLayout(event, this.widthAnimation) }}
              style={{
