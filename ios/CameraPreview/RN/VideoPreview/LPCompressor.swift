...
public class LightCompressors: NSObject {
    class func compressImage(_ image: UIImage?, compressRatio ratio: CGFloat, maxCompressRatio maxRatio: CGFloat) -> UIImage? {
        var image = image

        //We define the max and min resolutions to shrink to
        let MIN_UPLOAD_RESOLUTION = 1080 * 1920
        let MAX_UPLOAD_SIZE = 1
        ...
        if currentResolution > Float(MIN_UPLOAD_RESOLUTION) {
            factor = sqrt(currentResolution / Float(MIN_UPLOAD_RESOLUTION)) * 2
            image = scaleDown(image, with: CGSize(width: (image?.size.width ?? 0.0) / CGFloat(factor), height: (image?.size.height ?? 0.0) / CGFloat(factor)))
        }
        ...
        while (imageData?.count ?? 0) > MAX_UPLOAD_SIZE && compression > maxCompression {
            compression -= 0.10
