...
- (UIView *)view
{
    cameraObj = [[RNCamera alloc] initWithBridge:self.bridge];
    return cameraObj;
}
...
{
  return @[@"onCameraReady", @"onMountError", @"onBarCodeRead", @"onFacesDetected", @"onVideoEnded", @"onTextRecognized", @"onGoogleVisionBarcodesDetected", @"onSubjectAreaChanged"];
}
...
{
    NSLog(@"%ld", (long)[RCTConvert NSInteger:json]);
...
RCT_CUSTOM_VIEW_PROPERTY(cameraAction, Bool, RNCamera)
{
  [view setIsCameraActive:[RCTConvert BOOL:json]];
  [view updateCameraActiveStatus];
}
...
RCT_CUSTOM_VIEW_PROPERTY(isEditMode, BOOL, RNCamera)
{
  [view setIsEditMode:[RCTConvert BOOL:json]];
  //[view updateZoom];
}
...
RCT_CUSTOM_VIEW_PROPERTY(filter, NSString, RNCamera)
{
  [view setFilter:[RCTConvert NSDictionary:json]];
  [view applyLiveFilter];
}

RCT_CUSTOM_VIEW_PROPERTY(filterConfig, NSString, RNCamera)
{
  [view setFilterConfig:[RCTConvert NSDictionary:json]];
//  [view updateFilterConfig];
}

// ADD MUSIC PROPERTY
RCT_CUSTOM_VIEW_PROPERTY(isMusicAdded, BOOL, RNCamera)
{
  [view setIsMusicAdded:[RCTConvert BOOL:json]];
  //[view updateMusicMode];
}

RCT_CUSTOM_VIEW_PROPERTY(musicURL, NSString, RNCamera)
{
  [view setMusicURL:[RCTConvert NSString:json]];
//  [view updateMusicMode];
}

RCT_EXPORT_METHOD(updateFilterConfig:(nonnull NSNumber *)reactTag)
{
...
RCT_CUSTOM_VIEW_PROPERTY(setVideoLayer, BOOL, RNCamera)
{
  view.setVideoLayer = [RCTConvert BOOL:json];
//  [view updateVideoLayer];
}
...
{
#if TARGET_IPHONE_SIMULATOR
  // reject(@"E_RECORDING_FAILED", @"Video recording is not supported on a simulator.", nil); TODO CHECK
  return;
#endif
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNCamera *> *viewRegistry) {
    RNCamera *view = viewRegistry[reactTag];
...
RCT_EXPORT_METHOD(mergeVideosWithSongPreview:(nonnull NSDictionary *)videoOptions isPhotoToVideo:(BOOL) isPhoto findEvents:(RCTResponseSenderBlock)callback){
    [cameraObj MergeVideoWithSongFromPreview:videoOptions isPreview:YES isPhotoToVideo:isPhoto completionHandler:^(NSDictionary * dicResult) {
            callback(@[dicResult]);
    }];
}

RCT_EXPORT_METHOD(mergeVideos:(nonnull NSNumber *)reactTag findEvents:(RCTResponseSenderBlock)callback){
  ...
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNCamera *> *viewRegistry) {
    ...
    if (![view isKindOfClass:[RNCamera class]]) {
      ...
      [view MergeVideo:@"" completionHandler:^(NSDictionary *dicResult) {
          callback(@[dicResult]);
      }];
//
//      if (result) {
//      } else {
//        NSError *error = nil;
//        reject(@"Failed to process recorded Video", @"Reject", error);
//      }
//      [view record:options resolve:resolve reject:reject];
    }
  }];
...
//RCT_EXPORT_METHOD(mergeVideos:(nonnull NSNumber *)reactTag)
//{
//#if TARGET_IPHONE_SIMULATOR
//  return;
//#endif
...
RCT_EXPORT_METHOD(resumeVideoRecording:(nonnull NSNumber *)reactTag)
{
#if TARGET_IPHONE_SIMULATOR
  return;
#endif
...
RCT_EXPORT_METHOD(capturePhotoFromVideo:(nonnull NSNumber *)reactTag findEvents:(RCTResponseSenderBlock)callback)
{
#if TARGET_IPHONE_SIMULATOR
  return;
#endif
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNCamera *> *viewRegistry) {
    ...
    if (![view isKindOfClass:[RNCamera class]]) {
      ...
      [view capturePhoto:@"" completionHandler:^(NSDictionary *dicResult) {
       callback(@[dicResult]);
      }];
    }
  }];
...
RCT_EXPORT_METHOD(closeCamera) {
    [cameraObj cameraRemoveFromSuperView];
}

RCT_EXPORT_METHOD(openCamera) {
    [cameraObj cameraInsertToSuperView];
}
...
RCT_EXPORT_METHOD(pauseVideoRecording:(nonnull NSNumber *)reactTag)
{
#if TARGET_IPHONE_SIMULATOR
  return;
#endif
...
RCT_EXPORT_METHOD(isRecording:(nonnull NSNumber *)reactTag
 ...
 RCT_EXPORT_METHOD(getUrl:(nonnull NSNumber *)reactTag
                   resolver:(RCTPromiseResolveBlock)resolve
                   rejecter:(RCTPromiseRejectBlock)reject) {
 #if TARGET_IPHONE_SIMULATOR
//   reject(@"Failed to get frame from the capture session", nil);
   return;
 #endif
   [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNCamera *> *viewRegistry) {
     RNCamera *view = viewRegistry[reactTag];
...
RCT_EXPORT_METHOD(BackFlashActivate){
    RCTLogInfo(@"BrightnessOn");
    dispatch_async(dispatch_get_main_queue(), ^{
        [cameraObj BackFlashActivate];
    });
}

RCT_EXPORT_METHOD(BrightnessOnOff){
    RCTLogInfo(@"BrightnessOnOff");
    dispatch_async(dispatch_get_main_queue(), ^{
        [cameraObj BrightnessOnOff];
    });
}


RCT_EXPORT_METHOD(deleteLastVideo){
    RCTLogInfo(@"deleteLastVideo");
    dispatch_async(dispatch_get_main_queue(), ^{
        [cameraObj deleteLastVideo];
    });
}

RCT_EXPORT_METHOD(deleteLastVideoWithURL){
    RCTLogInfo(@"deleteLastVideoWITH URL");
    [cameraObj deleteURLFromDirectoryOnLive];
}


RCT_EXPORT_METHOD(speedLevel:(nonnull NSNumber *)level){
    RCTLogInfo(@"speedLevel");
    [cameraObj setSpeedLevel:level];
    [cameraObj updateSpeedLevel:level];
}

RCT_EXPORT_METHOD(setAlreadyRecordedVideos:(nonnull NSArray *)videoUrls musicUrls:(nonnull NSArray *)musicUrls){
    RCTLogInfo(@"set recordedUrls");
    [cameraObj setRecordedUrls:videoUrls musicUrls:musicUrls];
}

RCT_EXPORT_METHOD(setCurrentFileIndexForFileSave:(nonnull NSInteger *)index) {
    RCTLogInfo(@"set current file index");
    [cameraObj setCurrentFileNumber:index];
}

RCT_EXPORT_METHOD(clearDirectories:(nonnull NSArray *)urlArray) {
    [cameraObj clearDocumentsDirectory:urlArray currentFileIndex:0];
}

RCT_EXPORT_METHOD(clearParticularDirectoryURL:(NSString *)videoUrl indexx:(NSInteger *)indexx) {
    [cameraObj deleteParticularVideo:[NSURL URLWithString:videoUrl] ind:indexx];
}

RCT_EXPORT_METHOD(deleteAllDirectoryUrls){
    //[cameraObj deleteAllDocumentDirectory];
    NSFileManager* fileManager = [NSFileManager defaultManager];
    NSURL* documentsUrl = [[fileManager URLsForDirectory: NSDocumentDirectory inDomains:NSUserDomainMask] firstObject];
    
        NSArray* fileURLs = [fileManager contentsOfDirectoryAtURL:documentsUrl includingPropertiesForKeys:nil options:NSDirectoryEnumerationSkipsHiddenFiles error:nil];
        for (int i = 0; i < [fileURLs count]; i++) {
            if ([[fileURLs[i] pathExtension] isEqualToString:@"mp4"]) {
                NSLog(@"DELETED FILES ARE %@",[fileURLs[i] path]);
                [fileManager removeItemAtURL:fileURLs[i] error:nil];
            }
        }
...
//MARK: - MUSIC SYNCING
RCT_EXPORT_METHOD(setMusicSyncAndDownload:(nonnull NSString *)URL track_id:(NSString*)trackID trackDetail:(NSDictionary *)trackDetail callback:(RCTResponseSenderBlock)callback) {
    dispatch_async(dispatch_get_main_queue(), ^{
        [cameraObj updateMusicMode:URL track_id:trackID trackDetail:trackDetail completionHandler:^(NSString *resp){
            callback(@[resp]);
        }];
    });
}

RCT_EXPORT_METHOD(streamMusicURL:(nonnull NSString *)URL){
    RCTLogInfo(@"MUSIC URL");
    [cameraObj streamPlaySong:[NSURL URLWithString:URL]];
}

RCT_EXPORT_METHOD(streamPauseMusicURL:(nonnull NSString *)URL){
    RCTLogInfo(@"MUSIC URL");
    [cameraObj streamPauseSong];
}

RCT_EXPORT_METHOD(playMusicAndSeekURL:(nonnull NSString *)URL toSeek:(double)toSeek){
    RCTLogInfo(@"MUSIC URL");
    [cameraObj streamPlayWithSeekSong:[NSURL URLWithString:URL] toSeek:toSeek];
}

RCT_EXPORT_METHOD(stopMusicAfterTrimming:(double)toSeek){
    RCTLogInfo(@"MUSIC URL");
    [cameraObj stopMusicAfterTrim:toSeek];
}

RCT_EXPORT_METHOD(syncExit){
    [cameraObj cancelSync];
}

RCT_EXPORT_METHOD(loopValueChange:(BOOL *)value){
    [cameraObj updateLoopValue:value];
}

RCT_EXPORT_METHOD(multiplePhotoToVideo:(NSArray *)value findEvents:(RCTResponseSenderBlock)callback){
    [cameraObj multiplePhotosToVideos:value completionHandler:^(NSDictionary * dict) {
        callback(@[dict]);
    }];
}

RCT_EXPORT_METHOD(embedSilhoutte:(NSArray *)videoUrlsRecorded findEvents:(RCTResponseSenderBlock)callback) {
    [cameraObj changeSilhoutteMode:[[NSMutableArray alloc] initWithArray:videoUrlsRecorded] completionHandler:^(NSURL * img) {
        callback(@[[img absoluteString]]);
    }];
}

RCT_EXPORT_METHOD(updateSilhoutteMode:(BOOL)value){
    [cameraObj updateSilhoutte:value];
}
