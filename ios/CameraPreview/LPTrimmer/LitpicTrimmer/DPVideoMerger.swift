...
extension DPVideoMerger : VideoMerger {
    ...
    ///   - videoQuality: AVAssetExportPresetHighestQuality(default) , AVAssetExportPresetLowQuality , AVAssetExportPresetHighestQuality
    ...
    ///   - error: Gives Error object if some error occur in videos merging process
        ...
        open func mergeVideosWithTimer(withFileURLs
            videoFileURLs: [URL],
            ...
            for  videoFileURL in videoFileURLs {
                ...
                print("VIDEO DURATION",asset.duration)
                let silentAudioUrl = Bundle.main.url(forResource: "silence", withExtension: "mp3")!
                let aAudioAsset = AVAsset(url: silentAudioUrl)
                var audioAsset : AVAssetTrack? = nil
                
                if(asset.tracks(withMediaType: .audio).first != nil){
                    audioAsset = asset.tracks(withMediaType: .audio).first
                }else{
                    audioAsset = aAudioAsset.tracks(withMediaType: .audio).first
                }
                
                guard let audioAssett: AVAssetTrack = audioAsset else {
                    DispatchQueue.main.async {completion(nil, self.audioTarckError()) }
                    return
                }
                ...
                print("VIDEO TRIMMING TIME",CMTimeGetSeconds(trimmingTime),CMTimeGetSeconds((videoAsset.timeRange.duration)))
                ...
                do {
                    try videoTrack.insertTimeRange(videoTimeRange, of: videoAsset, at: currentTime)
                    try audioTrack.insertTimeRange(videoTimeRange, of: audioAssett, at: currentTime)
                    
                    let videoCompositionInstruction = AVMutableVideoCompositionInstruction.init()
                    
                    if(CMTimeGetSeconds(videoAsset.timeRange.duration) <= 4.5){
                        videoCompositionInstruction.timeRange = CMTimeRangeMake(start: CMTime.zero, duration: videoAsset.timeRange.duration)
                    }else{
                        if(CMTimeGetSeconds(trimmingTime).isNaN){
                            videoCompositionInstruction.timeRange = CMTimeRangeMake(start: currentTime, duration: videoTimeRange.duration)
                        }else{
                            videoCompositionInstruction.timeRange = CMTimeRangeMake(start: currentTime, duration: videoAsset.timeRange.duration)
                        }
                    }
...
//                layerInstruction.setTransform(Scale.concatenating(Move), at: CMTime.zero)
        ...
        open func singleVideoAudioMerge(withFileURLs
            videoFileURL: URL,
                                        videoAsset:AVAsset,
                                        audioAsset:AVAsset,
            videoResolution:CGSize = CGSize(width: -1, height: -1),
            ...
            let aAudioAsset = AVAsset(url: silentAudioUrl)
            var audioAssett : AVAssetTrack? = nil
            if(audioAsset.tracks(withMediaType: .audio).first !=  nil){
                audioAssett = audioAsset.tracks(withMediaType: .audio).first
            }else{
                audioAssett = aAudioAsset.tracks(withMediaType: .audio).first
            }
//                            guard let audioAsset: AVAssetTrack = audioAsset.tracks(withMediaType: .audio).first else {
//                                DispatchQueue.main.async {completion(nil, self.audioTarckError()) }
//                                return
//                            }
                            let currentFrameRate = Int(roundf((videoAsset.nominalFrameRate)))
            ...
            print("VIDEO ASSET TIME RANGE",CMTimeGetSeconds(videoAsset.timeRange.duration))
                            let trimmingTime: CMTime = CMTimeMake(value: Int64(lround(Double((videoAsset.nominalFrameRate) / (videoAsset.nominalFrameRate)))), timescale: Int32((videoAsset.nominalFrameRate)))
                            let timeRange: CMTimeRange = CMTimeRangeMake(start: trimmingTime, duration: CMTimeSubtract((videoAsset.timeRange.duration), trimmingTime))
                            do {
                                if(CMTimeGetSeconds(trimmingTime).isNaN){
                                    try videoTrack.insertTimeRange(videoAsset.timeRange, of: videoAsset, at: currentTime)
                                    try audioTrack.insertTimeRange(videoAsset.timeRange, of: audioAssett!, at: currentTime)
                                }else{
                                    try videoTrack.insertTimeRange(timeRange, of: videoAsset, at: currentTime)
                                    try audioTrack.insertTimeRange(timeRange, of: audioAssett!, at: currentTime)
                                }
        ...
        open func portraitConvertLandscapeWithMerge(withFileURLs
            videoFileURL: URL,
                                                    videoAsset:AVAsset,
                                                    audioAsset:AVAsset,
            videoResolution:CGSize = CGSize(width: 1080, height: 1920),
            videoQuality:String = AVAssetExportPresetHighestQuality,
            completion: @escaping (_ mergedVideoURL: URL?, _ error: Error?) -> Void) {
    ...
    fileprivate func generateMergedVideoFilePath() -> String {
        return URL(fileURLWithPath: ((FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).last)?.path)!).appendingPathComponent("\(UUID().uuidString)-mergedVideo.mp4").path
    }
